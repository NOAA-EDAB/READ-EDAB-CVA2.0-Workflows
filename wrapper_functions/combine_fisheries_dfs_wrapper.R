...
combine_fisheries_dfs_wrapper <- function(name, skip, force_overwrite = FALSE) {
  ...
  output_file <- file.path(here::here("SDMs"), name, 'input_csvs', 'combined_pa.csv')
  ...
  result <- tryCatch(
    {
      ...
      log_info(
        "{name}: Success. PA range: {paste(pa_range, collapse = ' to ')}"
      )

      write.csv(combinedDFs, row.names = F, file = output_file)
