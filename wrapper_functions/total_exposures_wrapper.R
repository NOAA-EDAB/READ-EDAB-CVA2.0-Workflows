...
total_exposures_wrapper <- function(
  ...
  # ==========================================================
  #ensemble weights
  load(file.path('../SDMs/', spp, 'model_output', 'ensemble_weights.rds')) #weights
  ...
  mapImp <- make_total_exposure(
    ...
    variable_exposure = mapExp,
    count_all = F,
    variable_weights = var_imp[
      nrow(var_imp),
      match(names(mapExp), colnames(var_imp))
    ], #last row is always the weighted average of the ensemble. reordered to match the names of the variable exposure rasters
  ...
  vecImp <- make_total_exposure(
    ...
    variable_weights = var_imp[
      nrow(var_imp),
      match(rownames(vecExp), colnames(var_imp))
    ], #last row is always the weighted average of the ensemble
    weight_threshold = 0.1
  )
  saveRDS(
    vecImp,
