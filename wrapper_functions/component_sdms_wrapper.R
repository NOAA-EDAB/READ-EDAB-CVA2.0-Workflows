...
component_sdms_wrapper <- function(
  spp,
  model,
  training_years,
  test_years,
  ...
  tryCatch(
    {
      ...
      s_names <- names(static_variables)[
        names(static_variables) %in% names(dfT)
      ]
      ...
      if (skip && model_exists) {
        log_warn(
          "Model file already exists for {spp}. Skipping model generation."
        )
        load(model_path)
      } else {
      ...
      if (skip && importance_exists) {
        log_warn(
          "Variable importance file already exists for {spp}. Skipping calculation."
        )
      } else {
        ...
        imp <- tryCatch(
          ...
          error = function(e) {
            log_error(
              "Failed to calculate variable importance for {spp}: {e$message}"
            )
            return(NULL)
          }
        )
      ...
      if (skip && predictions_exists) {
        log_warn(
          "{model} predictions already exist for {spp}. Skipping calculation."
        )
      } else {
        ...
        log_info(
          "{model} predictions for {spp} successfully saved to {predictions_path}"
        )
      }
      ...
      if (skip && evaluation_exists) {
        log_warn(
          "{model} has already been evaluated for {spp}. Skipping calculation."
        )
        ...
        log_info(
          "{model} evaluation for {spp} successfully saved to {evaluation_path}"
        )
      }
