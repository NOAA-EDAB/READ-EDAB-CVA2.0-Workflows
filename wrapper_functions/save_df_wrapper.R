...
save_df_wrapper <- function(
  ...
  if (skip && !force_overwrite && file.exists(output_file)) {
    log_info(
      "{csv_name} - {spp}: File exists and skip == TRUE. Skipping execution."
    )
    return(NA)
  }
  ...
  if (force_overwrite && file.exists(output_file)) {
    log_info(
      "{csv_name} - {spp}: File exists but force_overwrite == TRUE. Re-running."
    )
  }
  ...
  result <- tryCatch(
    {
      ...
      if (!file.exists(data_path)) {
        stop(paste("Source CSV missing:", data_path))
      }
      ...
      if (is.null(rast)) {
        log_warn(
          "{csv_name} - {spp}: rast is NULL - minimum conditions not met."
        )
        return(NULL)
      } else {
        ...
        log_info(
          "{csv_name} - {spp}: Success. PA range: {paste(pa_range, collapse = ' to ')}"
        )
